...
GeometricPrimitive::GeometricPrimitive(ID3D11Device* device)
{
	...
	// 1つのコントロールポイントの位置には法線の向きが違う頂点が 3 個あるので頂点情報の総数は 8x3=24 個、
...
// UNIT.12
GeometricSphere::GeometricSphere(ID3D11Device* device, uint32_t slices, uint32_t stacks) : GeometricPrimitive(device)
{
	std::vector<vertex> vertices;
	std::vector<uint32_t> indices;
...
// UNIT.12
GeometricCapsule::GeometricCapsule(ID3D11Device* device, float mantle_height, const DirectX::XMFLOAT3& radius, uint32_t slices, uint32_t ellipsoid_stacks, uint32_t mantle_stacks) : GeometricPrimitive(device)
{
	std::vector<vertex> vertices;
	std::vector<uint32_t> indices;

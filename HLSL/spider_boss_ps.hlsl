...
{
    // ディフューズマップをサンプリング
    float4 color = texture_maps[7].Sample(sampler_states[ANISOTROPIC], pin.texcoord);
    float alpha = color.a;
   ...
   // 垂直反射時のフレネル反射率
    ...
    float3 normal = texture_maps[6].Sample(sampler_states[LINEAR], pin.texcoord).xyz * 2.0f - 1.0f;

...
Texture2D basecolor_ : register(t13); 
Texture2D normal_ : register(t14); 
Texture2D roughness_ : register(t15); 
Texture2D metalness_ : register(t16);
...
{
    // ディフューズマップをサンプリング
    float4 color = basecolor_.Sample(sampler_states[ANISOTROPIC], pin.texcoord);
    float alpha = color.a;
   ...
   // 垂直反射時のフレネル反射率
    ...
    float3 normal = normal_.Sample(sampler_states[LINEAR], pin.texcoord).xyz * 2.0f - 1.0f;
    ...
    return float4((directDiffuse + directSpecular) * shadow_factor /*SHADOW*/, color.a);
